/* ================================================================
   ELITE ROUTE — ELITE ADMIN SUITE (V5.0 ULTRA)
   Features: Full CRUD, Custom Confirm/Reply Modals, Dashboard Stats,
             Image Compression, Dynamic Specs Builder, Keyboard Nav,
             Sister Companies, Footer Links, Premium UI/UX, Drag/Drop.
             + Enhanced Lightbox Image Gallery Viewer
================================================================ */
...
(function () {
    ...
    function initAdmin() {
        ...
        document.getElementById('car-form').onsubmit = (e) => handleFormSubmit(e, 'inventory', (fd) => {
            ...
            try { obj.imgs = JSON.parse(document.getElementById('imgs_hidden').value); } catch { obj.imgs = []; }
        ...
        // Primary Image Preview Lightbox interaction
    ...
    window.viewCarGallery = (id) => {
        ...
        if (car.imgs && Array.isArray(car.imgs)) {
            car.imgs.forEach(img => { if (!images.includes(img)) images.push(img); });
        }
        if (images.length === 0) {
    ...
    window.editForm = (table, id) => {
        ...
        if (table === 'inventory') {
            ...
            document.getElementById('imgs_hidden').value = JSON.stringify(item.imgs || []);
    ...
    window.viewInq = (id, table) => {
        ...
        d('inq-modal-vehicle-row').style.display = item.car_title || item.interest ? 'flex' : 'none';
        d('inq-modal-notes-row').style.display = item.notes || item.message ? 'flex' : 'none';
        d('inq-modal-dest-row').style.display = item.destination ? 'flex' : 'none';
    ...
    function setupImageUpload(fileId, urlId, hidId, prevId, multiple) {
        ...
        if (!multiple) {
            ui.addEventListener('input', e => { hi.value = e.target.value; pr.src = e.target.value; pr.style.display = e.target.value ? 'block' : 'none'; });
                ...
                ui.value = '';
                compressImage(e.target.files[0], b64 => { hi.value = b64; pr.src = b64; pr.style.display = 'block'; pr.style.cursor = 'pointer'; });
            });
        } else {
    ...
    function injectStyles() {
        ...
        .ep-nav-btn .ep-badge {
            ...
            background: var(--ep-primary); color: #fff; font-size: 0.6rem;
        ...
        /* ── LOGIN ── */
        #ep-login-form {
            padding: 80px 20px; max-width: 460px; margin: 0 auto;
            text-align: center; display: flex; flex-direction: column;
            align-items: center; gap: 0;
        }
        .ep-login-icon {
            width: 80px; height: 80px; background: rgba(255,59,48,0.1);
            border: 2px solid rgba(255,59,48,0.2); border-radius: 20px;
            display: flex; align-items: center; justify-content: center;
            font-size: 2rem; color: var(--ep-primary); margin-bottom: 25px;
        }
        .ep-login-title { font-size: 1.8rem; font-weight: 900; letter-spacing: 1px; margin-bottom: 8px; }
        .ep-login-sub { color: #666; font-size: 0.9rem; margin-bottom: 35px; }
        .ep-login-fields { width: 100%; display: flex; flex-direction: column; gap: 15px; margin-bottom: 25px; text-align: left; }
        ...
        .dash-card::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, rgba(255,59,48,0.05), transparent); pointer-events: none; }
        ...
        /* ── GRID/SECTION ── */
        .ep-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; }
        ...
        .modal-inner {
            ...
            opacity: 0; overflow: hidden; display: flex; flex-direction: column; max-height: 90vh;
        }
        ...
        /* ── INQUIRY MODAL ── */
        .inq-detail-row { display: flex; gap: 12px; align-items: flex-start; padding: 14px 0; border-bottom: 1px solid rgba(255,255,255,0.04); }
        .inq-detail-row:last-of-type { border-bottom: none; }
        .inq-detail-icon { width: 36px; height: 36px; background: rgba(255,59,48,0.08); border: 1px solid rgba(255,59,48,0.15); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: var(--ep-primary); font-size: 0.95rem; flex-shrink: 0; margin-top: 2px; }
        .inq-detail-label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 1.5px; color: #666; font-weight: 700; margin-bottom: 4px; }
        .inq-detail-value { font-size: 0.95rem; color: #e8e8e8; line-height: 1.5; }

        /* ── CONFIRM MODAL ── */
        .confirm-icon-wrap { width: 70px; height: 70px; background: rgba(255,59,48,0.1); border: 2px solid rgba(255,59,48,0.2); border-radius: 20px; display: flex; align-items: center; justify-content: center; font-size: 2rem; color: var(--ep-primary); margin: 0 auto 22px; }
        .confirm-title { font-size: 1.4rem; font-weight: 900; text-align: center; margin-bottom: 12px; }
        .confirm-msg { color: #888; text-align: center; line-height: 1.6; font-size: 0.95rem; }
        .confirm-actions { display: flex; gap: 14px; margin-top: 30px; justify-content: center; }
        ...
        /* ── TOAST ── */
        #ep-toast-container { position: fixed; bottom: 28px; right: 28px; z-index: 13000; display: flex; flex-direction: column; gap: 10px; pointer-events: none; }
        .ep-toast {
    ...
    function injectHTML() {
            ...
            <div class="ep-container">
                ...
                <div class="ep-header">
                    <div class="ep-header-brand">
                        ...
                        <div class="logo-badge">Pro Suite V5</div>
                    </div>
                ...
                <div id="ep-main-panel" style="display:none; flex-direction:column; flex:1; overflow:hidden; min-height:0;">
                    ...
                    <div class="ep-content">
                        ...
                        <div id="tab-dashboard" class="ep-tab-content active">
                            <div class="dash-grid">
                                <div class="dash-card">
                                    <div class="dash-card-icon"><i class="fas fa-car-side"></i></div>
                                    <div class="dash-card-value" id="dash-inventory">—</div>
                                    <div class="dash-card-label">Vehicles Listed</div>
                                </div>
                                <div class="dash-card" style="cursor:pointer;" onclick="document.querySelector('[data-tab=tab-leads]').click()">
                                    <div class="dash-card-icon"><i class="fas fa-envelope-open-text"></i></div>
                                    <div class="dash-card-value" id="dash-inquiries">—</div>
                                    <div class="dash-card-label">Total Inquiries</div>
                                    <div class="dash-card-sub" id="dash-inquiries-new">Loading…</div>
                                </div>
                                <div class="dash-card" style="cursor:pointer;" onclick="document.querySelector('[data-tab=tab-quotes]').click()">
                                    <div class="dash-card-icon"><i class="fas fa-file-invoice-dollar"></i></div>
                                    <div class="dash-card-value" id="dash-quotes">—</div>
                                    <div class="dash-card-label">Quote Requests</div>
                                    <div class="dash-card-sub" id="dash-quotes-new">Loading…</div>
                                </div>
                                <div class="dash-card">
                                    <div class="dash-card-icon"><i class="fas fa-briefcase"></i></div>
                                    <div class="dash-card-value" id="dash-services">—</div>
                                    <div class="dash-card-label">Active Services</div>
                                </div>
                            </div>
                            ...
                            <div class="tab-toolbar">
                                <div class="tab-toolbar-left">
                                    <div class="tab-search-wrap">
                                        ...
                                        <input type="text" id="inv-search" class="ep-input" placeholder="Search make or model…" onkeyup="window.filterInventory()" style="width:300px">
                                    </div>
                                </div>
                            ...
                            <div class="split-grid">
                                <div class="ep-section">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
                                        <div class="ep-sec-title" style="margin:0;border:none;padding:0;"><i class="fas fa-building"></i> Sister Companies</div>
                                ...
                                <div class="ep-section">
                                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
                                        <div class="ep-sec-title" style="margin:0;border:none;padding:0;"><i class="fas fa-link"></i> Footer Links</div>
                        ...
                        <div id="tab-settings" class="ep-tab-content">
                            <div class="ep-section" style="max-width:640px;">
                                ...
                                <form id="settings-form" style="display:flex;flex-direction:column;gap:18px;">
                                    <div><label class="ep-label">Physical Address</label><input id="set-address" class="ep-input" placeholder="e.g. Office 1402, Conrad Business Tower, Dubai, UAE"></div>
                                    <div><label class="ep-label">Phone / WhatsApp</label><input id="set-phone" class="ep-input" placeholder="+971 55 000 0000"></div>
                                    <div><label class="ep-label">Support Email</label><input id="set-email" class="ep-input" placeholder="export@eliteroute.ae"></div>
                                    <div><label class="ep-label">Google Maps Embed URL</label><textarea id="set-map" class="ep-textarea" placeholder="Paste the src URL from Google Maps embed code…"></textarea></div>
            ...
            <div class="modal-inner">
                ...
                <form id="car-form" style="display:contents;">
                    <div class="modal-header">
                        ...
                        <p>Fill in all required fields. Images are auto-compressed on upload. Click any image to view in full resolution.</p>
                    </div>
                            ...
                            <div class="split-grid">
                                <div>
                                    ...
                                    <div class="file-drop-area">
                                        <i class="fas fa-image" style="font-size:28px;color:#ff3b30;margin-bottom:10px;"></i>
                                        <div style="font-size:0.82rem;font-weight:700;color:#fff;margin-bottom:4px;">Select or Drop Photo</div>
                                        <div style="font-size:0.72rem;color:#666;">Auto-compressed to max 1400px</div>
                                    ...
                                    <img id="img_preview" style="width:100%;height:150px;object-fit:cover;display:none;margin-top:12px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);transition:0.3s;cursor:pointer;" onmouseover="this.style.borderColor='var(--ep-primary)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.08)'" title="Click to view full size">
                                </div>
                                <div>
                                    <label class="ep-label">Gallery Images (multiple)</label>
                                    <div class="file-drop-area">
                                        <i class="fas fa-cloud-upload-alt" style="font-size:28px;color:#ff3b30;margin-bottom:10px;"></i>
                                        <div style="font-size:0.82rem;font-weight:700;color:#fff;margin-bottom:4px;">Drop Multiple Photos</div>
                                        <div style="font-size:0.72rem;color:#666;">All images auto-compressed</div>
                                    ...
                                    <div id="imgs_preview" class="img-preview-grid" style="margin-top:14px;"></div>
                                </div>
                            </div>
                        </div>

                        <div class="ep-section">
                            <div class="ep-sec-title"><i class="fas fa-sliders-h"></i> Vehicle Specifications</div>
                            <div id="specs-container"></div>
                            <button type="button" class="ep-btn ep-btn-sec ep-btn-sm" style="margin-top:4px;" onclick="window.addKVRow('specs-container')">
                                <i class="fas fa-plus"></i> Add Spec Row
                            </button>
                        </div>

                        <div class="ep-section">
                            <div class="ep-sec-title"><i class="fas fa-ship"></i> Export Information</div>
                            <div id="export-container"></div>
                            <button type="button" class="ep-btn ep-btn-sec ep-btn-sm" style="margin-top:4px;" onclick="window.addKVRow('export-container')">
                                <i class="fas fa-plus"></i> Add Export Row
                            </button>
                        </div>

                        <div class="ep-section">
                            <div class="ep-sec-title"><i class="fas fa-align-left"></i> Description</div>
                            <div style="display:flex;flex-direction:column;gap:16px;">
                                <div><label class="ep-label">Full Description</label><textarea name="description" class="ep-textarea" placeholder="Detailed vehicle description…" style="min-height:140px;"></textarea></div>
                                <div class="split-grid">
                                    <div><label class="ep-label">Year</label><input name="year" class="ep-input" placeholder="e.g. 2024"></div>
                                    <div><label class="ep-label">Mileage</label><input name="mileage" class="ep-input" placeholder="e.g. 0 km (Brand New)"></div>
                                </div>
                                <div class="split-grid">
                                    <div><label class="ep-label">Color</label><input name="color" class="ep-input" placeholder="e.g. Obsidian Black"></div>
                                    <div><label class="ep-label">Order Index</label><input name="order_idx" type="number" class="ep-input" placeholder="e.g. 1 (lower = first)"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="ep-btn ep-btn-sec" onclick="window.closeAdminModal('ep-car-modal')"><i class="fas fa-times"></i> Cancel</button>
            ...
            <div class="modal-inner" style="max-width:600px;">
                ...
                <form id="service-form" style="display:contents;">
                    <div class="modal-header">
                        ...
                        <p>Add or update a service offering shown on the website.</p>
                    </div>
                    <div class="modal-body-scroll">
                        <div style="display:flex;flex-direction:column;gap:18px;">
                            <div><label class="ep-label">Service Title</label><input name="title" class="ep-input" required placeholder="e.g. Vehicle Export"></div>
                            <div><label class="ep-label">Font Awesome Icon Class</label><input name="icon" class="ep-input" placeholder="e.g. fas fa-ship"></div>
                            <div><label class="ep-label">Short Description</label><textarea name="description" class="ep-textarea" style="min-height:100px;" placeholder="Brief description of the service…"></textarea></div>
                            <div><label class="ep-label">Order Index</label><input name="order_idx" type="number" class="ep-input" placeholder="e.g. 1"></div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="ep-btn ep-btn-sec" onclick="window.closeAdminModal('ep-service-modal')"><i class="fas fa-times"></i> Cancel</button>
            ...
            <div class="modal-inner" style="max-width:560px;">
                ...
                <form id="sister-form" style="display:contents;">
                    <div class="modal-header">
                        ...
                        <p>Manage associated companies displayed in the footer.</p>
                    </div>
                    <div class="modal-body-scroll">
                        <div style="display:flex;flex-direction:column;gap:18px;">
                            <div><label class="ep-label">Highlight Word (colored prefix)</label><input name="highlight" class="ep-input" placeholder="e.g. ELITE"></div>
                            <div><label class="ep-label">Company Name</label><input name="name" class="ep-input" required placeholder="e.g. Route Logistics"></div>
                            <div><label class="ep-label">Website URL</label><input name="url" type="url" class="ep-input" placeholder="https://example.com"></div>
                            <div><label class="ep-label">Order Index</label><input name="order_idx" type="number" class="ep-input" placeholder="e.g. 1"></div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="ep-btn ep-btn-sec" onclick="window.closeAdminModal('ep-sister-modal')"><i class="fas fa-times"></i> Cancel</button>
            ...
            <div class="modal-inner" style="max-width:560px;">
                ...
                <form id="footer-link-form" style="display:contents;">
                    <div class="modal-header">
                        ...
                        <p>Add or update a link in the website footer navigation.</p>
                    </div>
                    <div class="modal-body-scroll">
                        <div style="display:flex;flex-direction:column;gap:18px;">
                            <div><label class="ep-label">Section / Group</label><input name="section" class="ep-input" required placeholder="e.g. Company, Services, Contact"></div>
                            <div><label class="ep-label">Link Label</label><input name="label" class="ep-input" required placeholder="e.g. About Us"></div>
                            <div><label class="ep-label">Font Awesome Icon Class</label><input name="icon" class="ep-input" placeholder="e.g. fas fa-info-circle"></div>
                            <div><label class="ep-label">URL</label><input name="url" class="ep-input" placeholder="e.g. /about or https://…"></div>
                            <div><label class="ep-label">Order Index</label><input name="order_idx" type="number" class="ep-input" placeholder="e.g. 1"></div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="ep-btn ep-btn-sec" onclick="window.closeAdminModal('ep-footer-link-modal')"><i class="fas fa-times"></i> Cancel</button>
        ...
        <div id="ep-inq-modal" class="ep-modal-wrapper" style="display:none;">
            <div class="modal-inner" style="max-width:680px;">
                ...
                <div class="modal-header">
                    <h2>Lead Details</h2>
                    <p>Review customer information and send an internal reply note.</p>
                </div>
                <div class="modal-body-scroll">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:22px;flex-wrap:wrap;gap:12px;">
                        <div>
                            <div style="font-size:1.5rem;font-weight:900;" id="inq-modal-name">—</div>
                            <div style="color:#666;font-size:0.82rem;margin-top:3px;" id="inq-modal-date">—</div>
                        </div>
                        <div id="inq-modal-status"></div>
                    </div>

                    <div class="inq-detail-row">
                        <div class="inq-detail-icon"><i class="fas fa-envelope"></i></div>
                        <div><div class="inq-detail-label">Email</div><div class="inq-detail-value" id="inq-modal-email">—</div></div>
                    </div>
                    <div class="inq-detail-row">
                        <div class="inq-detail-icon"><i class="fas fa-phone"></i></div>
                        <div><div class="inq-detail-label">Phone</div><div class="inq-detail-value" id="inq-modal-phone">—</div></div>
                    </div>
                    <div class="inq-detail-row" id="inq-modal-dest-row">
                        <div class="inq-detail-icon"><i class="fas fa-map-marker-alt"></i></div>
                        <div><div class="inq-detail-label">Destination</div><div class="inq-detail-value" id="inq-modal-dest">—</div></div>
                    </div>
                    <div class="inq-detail-row" id="inq-modal-vehicle-row">
                        <div class="inq-detail-icon"><i class="fas fa-car"></i></div>
                        <div><div class="inq-detail-label">Vehicle / Interest</div><div class="inq-detail-value" id="inq-modal-vehicle">—</div></div>
                    </div>
                    <div class="inq-detail-row" id="inq-modal-notes-row">
                        <div class="inq-detail-icon"><i class="fas fa-sticky-note"></i></div>
                        <div><div class="inq-detail-label">Message / Notes</div><div class="inq-detail-value" id="inq-modal-notes">—</div></div>
                    </div>

                    <div style="margin-top:28px;">
                        <label class="ep-label"><i class="fas fa-reply" style="margin-right:6px;"></i>Internal Reply / Notes</label>
                        <textarea id="inq-modal-reply" class="ep-textarea" placeholder="Add your reply or internal notes here…" style="min-height:110px;margin-top:10px;"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="ep-btn ep-btn-sec" onclick="window.closeAdminModal('ep-inq-modal')"><i class="fas fa-times"></i> Close</button>
                    <button type="button" id="ep-inq-reply-btn" class="ep-btn ep-btn-p"><i class="fas fa-check-circle"></i> Save & Mark Replied</button>
                </div>
            </div>
        </div>
        ...
        <div id="ep-confirm-modal" class="ep-modal-wrapper" style="display:none;">
            <div class="modal-inner" style="max-width:460px;">
                <div style="padding:42px 38px;text-align:center;">
                    <div class="confirm-icon-wrap"><i class="fas fa-exclamation-triangle"></i></div>
                    <div class="confirm-title" id="ep-confirm-title">Confirm Action</div>
                    <div class="confirm-msg" id="ep-confirm-msg">Are you sure you want to proceed?</div>
                    <div class="confirm-actions">
                        <button id="ep-confirm-cancel" class="ep-btn ep-btn-sec" style="min-width:130px;"><i class="fas fa-times"></i> Cancel</button>
                        <button id="ep-confirm-ok" class="ep-btn ep-btn-danger" style="min-width:130px;"><i class="fas fa-trash"></i> Delete</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <button id="ep-admin-trigger" title="Open Admin Panel" aria-label="Open Admin Panel">
